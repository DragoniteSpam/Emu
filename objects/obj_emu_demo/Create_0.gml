...
#region Right container
	containerR = new EmuCore(room_width - room_width / 4, 32, room_width / 4, 640);
	
	var tab_groupR = new EmuTabGroup(0, 0, containerR.width, containerR.height, 2, 32);
	var tab_temp = new EmuTab("Temp");
	
	tab_groupR.AddTabs(0, tab_temp);
	
	containerR.AddContent(tab_groupR);
#endregion

#region 3D view container
	containerPreview = new EmuCore(room_width / 4, room_height/2 - (room_width/2) * 400 / 540 / 2, room_width / 2, (room_width/2) * 400 / 540);
	containerPreview.AddContent(
	    new EmuRenderSurface(0, 0, containerPreview.width, containerPreview.height,
